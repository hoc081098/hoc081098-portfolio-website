...
- Nếu `elementData === emptyElementData` - ở đây `emptyElementData` là một **sentinel value** để đại diện cho một `ArrayDeque` mới khởi tạo mà chưa có capacity nào được allocated (via `constructor()`) - thì sẽ tạo một array mới với capacity là max của `minCapacity` và `defaultMinCapacity = 10`.
