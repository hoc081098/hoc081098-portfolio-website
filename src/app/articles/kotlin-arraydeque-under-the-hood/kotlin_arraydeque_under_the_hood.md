...
- `lastIndex` là logical index của phần tử cuối. Gọi `internalIndex(lastIndex)` để convert sang _physical index_ tương ứng trong backing array, lưu vào `internalLastIndex`. Sau đó, dùng `internalGet(internalLastIndex)` để đọc giá trị phần tử tại vị trí đó, lưu vào `element` để trả về cho caller.
