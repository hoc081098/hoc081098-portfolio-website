...
export const article = {
  author: 'hoc081098',
  date: '2026-02-14',
  title: 'Rate limiting strategies for Nginx, API Gateway, and service layers',
  description:
...
export const metadata = {
  ...
  openGraph: {
    images: [
      `/api/og?type=article&title=${encodeURIComponent('Rate limiting strategies for Nginx, API Gateway, and service layers')}&description=${encodeURIComponent('Rate limiting không nên đặt ở một tầng duy nhất. Phân tích chiến lược rate limiting tại Nginx (edge), API Gateway (YARP), và service layer trong ASP.NET Core.')}`,
    ],
  },
}
...
[![Hits](https://hits.sh/portfolio.hoc081098.dev/articles/rate-limiting-strategies.svg)](https://hits.sh/portfolio.hoc081098.dev/articles/rate-limiting-strategies/)
...
- Sliding Window: Cải tiến hơn Fixed Window. Giới hạn N request/X giây gần nhất bằng cách chia Window thành nhiều segment cố định, mỗi khi check sẽ đếm số request của segment hiện tại với các segment trước đó.
  ...
  Đọc thêm tại [bài viết trên Facebook](https://www.facebook.com/hoc081098/posts/pfbid02XBGAAW7iu8SymptgHhxERbfwri4RQGNGJaJ4kR72vv4iUu6bGTtUJ6q3HPbQP2Wgl)
- Token Bucket: Mỗi request sẽ "rút" token từ bucket. Bucket được refill theo thời gian.
  ...
  Đọc thêm tại [bài viết trên Facebook](https://www.facebook.com/hoc081098/posts/pfbid02s3RrBtD93FminminepcLM4GBhkADZ95bmjwz9GXWAsYMWDgXUc1uRnVYuwRwojRwl)
...
Đọc thêm [Microsoft docs: Rate Limiting in YARP](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/servers/yarp/rate-limiting?view=aspnetcore-10.0)
