...
Trước khi đi vào type signature, ta hãy xem một flow load user.
API có thể fail vì mất mạng, hết session hoặc lỗi bất ngờ.
Nếu mất mạng, ta thử đọc cache, nếu vẫn không có dữ liệu ở cache, thì error còn lại được chuyển thành một UI state phù hợp.
...
- `map` đổi `User` thành content UI model khi pipeline thành công.
