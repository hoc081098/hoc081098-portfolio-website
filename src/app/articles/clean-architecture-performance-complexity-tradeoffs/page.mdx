...
- _DB connection pool starvation_ và _cache stampede_ là những failure mode có thể ảnh hưởng đến
...
- Cache là một trade-off riêng, không phải giải pháp mặc định cho GC pressure. Nó phù hợp hơn với
  read-heavy workloads và những use case chấp nhận dữ liệu cũ trong một khoảng ngắn (_bounded staleness_).
