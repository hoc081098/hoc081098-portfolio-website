...
export const metadata = {
  ...
  openGraph: {
    images: [
      `/api/og?type=article&title=${encodeURIComponent('Union Types in C# — Discriminated Unions with OneOf')}&description=${encodeURIComponent('How to model union types and discriminated unions in C# using the OneOf library, with practical examples for error handling and domain modeling.')}`,
    ],
  },
}
...
[![Hits](https://hits.sh/portfolio.hoc081098.dev/articles/csharp-union-types.svg)](https://hits.sh/portfolio.hoc081098.dev/articles/csharp-union-types/)
